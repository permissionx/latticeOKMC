...
# ╔═╡ 3a7a51f4-ca10-11ec-1732-a9153e2f29b6
using Plots, Measures,PlutoUI

# ╔═╡ 2adcb3a5-5c72-488c-8007-770b39fe3af6
using Rotations

# ╔═╡ 11dd1a2a-ccca-4857-b2b0-cb99514857ea
theme(:juno)

# ╔═╡ e0426691-1b4a-425c-9e4d-b31a094df763
function plotline(points::Matrix{Int64}; color::Int64=1, ls=:solid)
    plot(points[:,1], points[:,2], points[:,3], color = color, ls=ls, lw=2,
    zlim=(-2,2), xlim=(-2,2), ylim=(-2,2),aspect_ratio=:equal,legend= false, showaxis = false, ticks = false,background_color_inside=nothing,
     size = (300,300))
end

# ╔═╡ f7903378-3626-447c-92ff-7a15dc70d3e8
function plotline!(points::Matrix{Int64}; color::Int64=1, ls=:solid)
    plot!(points[:,1], points[:,2], points[:,3], color = color, ls=ls, lw=2)
end

# ╔═╡ b0eda70e-9b2a-4629-9b37-a4c3270f6420
function background()
	...
	scatter!(points[:,1], points[:,2], points[:,3], markeredgestyle=:dash,ms=5,msw=1,mc=:black)
	scatter!([0], [0], [0], mc=:white)
end

# ╔═╡ 3c98da7c-2aed-4f1a-a65e-2b056a419f3e
background()

# ╔═╡ c5680f8e-7ea8-4491-9b45-2954aef19d92
Rs = AngleAxis[]

# ╔═╡ 979e94d6-6915-406f-8b7c-bf7c6b3fa4a8
begin
...
# ╔═╡ 497d296a-0098-40a5-ad5a-d9a495fd988a
...
# ╔═╡ 65598f01-92c9-4f15-b9b7-0c8abcfbe781
begin
...
# ╔═╡ 0c53720a-4962-4755-b704-2e4464c88282
float_points_set = all_combinations()

# ╔═╡ d695aae8-93a4-4ef8-8178-9ee1861fe482
points_set = Vector{Matrix{Int64}}([])

# ╔═╡ 22eba11d-2a9c-438c-a749-93b90db5e11b
for i in 1:length(float_points_set)
    push!(points_set, sortslices(round.(Int, float_points_set[i]), dims=1, by=x->(x[1],x[2],x[3])))
end

# ╔═╡ b40142b4-6fbe-49fe-8745-b410b5b9eda3
points_set

# ╔═╡ 71df5209-5eab-49da-b462-7c81c5a8e411
i = 1

# ╔═╡ a3c0a716-4a79-45df-a38d-2d1e088fca9d
while true
...
# ╔═╡ d7d9980f-349f-4308-8d1d-b376180e848f
function plot_points(points::Matrix{Int64}, n::Int64)
    background()
    scatter!(points[:,1], points[:,2], points[:,3], msw=0,  mc=:white, msc=5, title="\n\n$(n)",titlefontsize=12, titlefontcolor=:white)
end

# ╔═╡ 0e5b385e-8e3e-4f39-af6e-92eb761ca62c
html"""
<style>
input[type*="range"] {
	width: 95%;
...
# ╔═╡ 44e30300-297e-4a61-8ac2-c3e6f965a109
@bind x Slider(1:554-19)

# ╔═╡ 2456e8c6-37c5-4dc4-a67c-f08b6445c228
md"""
_Vacancy sets from $(x)th to $(x+19)th_
"""

# ╔═╡ 2e8818a2-9357-41ac-b287-e35b28235afc
begin
ps = []
for x in x:x+19
	points = points_set[x]
    p = plot_points(points,x)
    push!(ps, p)
end
#push!(ps, plot())
plot(ps..., layout= (4,5),size=(1000,1000))
end

# ╔═╡ 27099d74-50fc-4407-b512-c8e8b546d211
@bind n Slider(1:554)

# ╔═╡ 9990889c-deac-4f97-9912-c3fe4d3baec8
md"""
_$(n)th of sets_
"""

# ╔═╡ b82a6822-3d29-4f86-b895-009fa53801bb
plot_points(points_set[n],n)

# ╔═╡ 643c35a0-347f-42f8-8151-e7f0d298c124
@bind m Slider(0:1000)

# ╔═╡ 7806e3e6-63a9-4f1b-8ef0-b3de8e7007fb
begin
x1 = 0:0.1:m/200*pi
y1 = sin.(x1)
y2 = cos.(x1)
...
# ╔═╡ 86d02985-0945-42a4-af3a-aae66f2fc133
begin
plot(x1,y1,xlim=(0,5pi), ylim=(-1.1,1.1),lw=2,label="sin")
plot!(x1,y2,xlim=(0,5pi), ylim=(-1.1,1.1),lw=2,label="cos")
end
